    ...
    <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
        <Box sx={{ display: 'flex', gap: 2, width: '100%', maxWidth: 1200 }}>
            ...
            <Paper sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap:1 }}>
                  <IconButton color="primary" onClick={handleLike}><ThumbUpIcon /></IconButton>
                  <Typography>{likeCount}</Typography>
                  <IconButton color="error" onClick={handleUnlike}><ThumbDownIcon /></IconButton>
              ...
              <Box sx={{ mb: 3, display: 'flex', justifyContent: 'center' }}>
                <img
                  ...
                  alt={blog.title}
                  style={{ width: 600, maxHeight: 400, objectFit: 'cover', borderRadius: 8 }}
                />
              </Box>

              <Box
                sx={{ mt: 2, mb: 4, lineHeight: 1.7, fontSize: '1.1rem', '& p': { marginBottom: '1.5rem' } }}
          ...
          {/* Right Side: Comments */}
          {openComment && (
            <Paper sx={{ flex: 1.2, p: 2, height: 'calc(100vh - 100px)', overflowY: 'auto', position: 'sticky', top: 80 }}>
                    ...
                    {repliesMap[comment.id] && repliesMap[comment.id].length > 0 && (
                      <Box sx={{ ml: 4, mt: 1 }}>
                        {repliesMap[comment.id].map((reply) => (
                          <Box key={reply.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
